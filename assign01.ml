...
(*** YOUR NAME HERE Zhenhuan Cui ***)
(*** YOUR STUDENT ID HERE 300055809***)
(*** OCAML VERSION USED FOR THIS ASSIGNMENT HERE 4.12.0***)
...
(* 1e. Replace the .... with one thing you hope to learn about in
   this course *)
let learning : string = "I hope to learn Ocaml"
...
let prob3a = let x = 4 in
             let y = 3.9 in
             float_of_int(2 * x) +. (3. *. y)


let exp3a : string = "1.'*' is for int multiplications but here y has type float 
                        2. x has type int and y has type float, so there will be a type mismatch when adding them together"
...
let exp3b1 : string = "k-1 should be in parentheses"

let exp3b2 : string = "arugment types should be included after let rec exp"
...
let exp4 : string = "this function will never end"
...
let rec is_palindrome(s:string):bool = 
   ...
   else if String.length s = 2 && String.get s ((String.length s)-1) = (String.get s 0) then true
   else if String.get s ((String.length s)-1) = (String.get s 0) && is_palindrome(String.sub s 1 ((String.length s)-2)) then true
else false
